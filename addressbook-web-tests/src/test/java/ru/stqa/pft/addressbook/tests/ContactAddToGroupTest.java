...
public class ContactAddToGroupTest extends TestBase {
    ...
    public void ensurePreconditions (){
       ...
       Groups contactGroups = addedToGroupContact.getGroups();
       GroupData newGroup = new GroupData().withName("test5").withHeader("header").withFooter("footer");
            if (groups.size()==contactGroups.size()){
            app.goTo().groupPage();
            app.group().create(newGroup);
        } /*else if (contactGroups.contains(addedGroup)){
