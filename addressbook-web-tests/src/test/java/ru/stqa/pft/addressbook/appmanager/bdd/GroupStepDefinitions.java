package ru.stqa.pft.addressbook.appmanager.bdd;
...
import org.python.apache.xerces.impl.xs.identity.Selector;
...
public class GroupStepDefinitions  {
    ...
    public void verfiyGroupCreated(){
        Groups newGroups = app.db().groups();
        assertThat(newGroups, equalTo(groups.withAdded(newGroup.
                withId(newGroups.stream().mapToInt((g) -> g.getId()).max().getAsInt()))));
